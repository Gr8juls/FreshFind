        ...
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          <Link href="/" className="flex items-center space-x-3 cursor-pointer shrink-0">
            ...
            <div>
              ...
              <span className="hidden md:inline-block ml-2 text-[11px] font-semibold px-2 py-0.5 rounded-full bg-brand-500/10 text-brand-600 dark:text-brand-400 border border-brand-500/20">
                Food Rescue
              </span>
            </div>
          </Link>
          ...
          {activeRole === 'CUSTOMER' && (
            <div className="hidden md:flex flex-1 max-w-md relative">
              ...
              <input
                ...
                placeholder="Search bakeries, cafes, meals in Kigali..."
          ...
          {activeRole === 'CUSTOMER' && (
            <div className="hidden xl:flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-3 py-1 rounded-xl border border-emerald-200 dark:border-emerald-500/30">
                ...
                <span>{user.mealsRescued || 18} Saved • {user.co2SavedKg || 45}kg CO₂</span>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center space-x-2.5 shrink-0">
            ...
            {isAuthenticated && (user.role === 'BUSINESS_OWNER' || user.role === 'ADMIN') && !pathname.startsWith('/business') && (
              <Link
                href="/business"
                className="flex items-center space-x-1.5 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 border border-brand-500/30 text-brand-600 dark:text-brand-400 px-3 py-1.5 rounded-xl text-xs font-bold transition"
              >
            ...
            {isAuthenticated && user.role === 'ADMIN' && !pathname.startsWith('/admin') && (
              <Link
                href="/admin"
                className="flex items-center space-x-1.5 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 border border-purple-500/30 text-purple-600 dark:text-purple-400 px-3 py-1.5 rounded-xl text-xs font-bold transition"
              >
              ...
              <div className="flex items-center space-x-2 pl-2 border-l border-slate-200 dark:border-slate-800">
                <div className="hidden sm:flex flex-col items-end">
                  ...
                  <span className="text-[10px] font-semibold text-brand-600 dark:text-brand-400">
                    {user.role === 'BUSINESS_OWNER' ? 'Merchant' : user.role === 'ADMIN' ? 'Admin' : 'Shopper'}
                  </span>
                </div>
                <button
                  onClick={logout}
